...
class Linux(LinuxMixin):
    ...
    def scrub(snapshot_data: Dict[str, Any], span: Span) -> bool:
        ...
        if not all(template_data[key] is not None for key in Linux.template_keys):
            ...
            error_msg = (
                f'Template Data Error, the following keys were missing from the Snapshot scrub data: '
                f'{", ".join(missing_keys)}.'
            )
        ...
        try:
            # Try connecting to the host and running the necessary command
            client.connect(hostname=host_ip, username='administrator')  # No need for passwords as it should have keys
