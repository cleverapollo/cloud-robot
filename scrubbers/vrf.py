...
class Vrf:
    ...
    logger = utils.get_logger_for_name('scrubbers.vrf')

    @staticmethod
    def scrub(vrf: dict, password: str) -> bool:
        """
        Using the JunOS python library, this method attempts to scrub a Virtual Router in a Physical Router using the
        config passed in from the dispatcher, and reports back to the dispatcher whether or not the scrubbed succeeded
        :param vrf: The VRF data generated by the dispatcher
        :param password: The network password for connecting to the router
        :return: A flag stating whether the scrub succeeded or not
        """
