...
class Vrf(VrfMixin):
    ...
    template_keys = {
        ...
        'management_ip',
        # The id of the Project that owns the VRF being quiesced
        'project_id',
    }

    @staticmethod
    def quiesce(vrf_data: Dict[str, Any], span: Span) -> bool:
        """
        Commence the quiesce of a vrf using the data read from the API
        ...
        if not all(template_data[key] is not None for key in Vrf.template_keys):
            ...
            Vrf.logger.error(
                f'Template Data Error, the following keys were missing from the VRF quiesce data: '
                f'{", ".join(missing_keys)}',
            )
