...
class Windows(WindowsMixin):
    ...
    logger = logging.getLogger('quiescers.vm.windows')
    ...
    def quiesce(vm_data: Dict[str, Any]) -> bool:
        ...
        vm_quiesce_cmd = utils.JINJA_ENV.get_template('vm/windows/quiesce_cmd.j2').render(**template_data)

        # Open a client and run the two necessary commands on the host
        quiesced = False
        try:
            response = Windows.deploy(vm_quiesce_cmd, host_name)
        except WinRMError:
            Windows.logger.error(
