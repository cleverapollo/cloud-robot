...
class Windows(WindowsMixin):
    ...
    def quiesce(vm_data: Dict[str, Any], span: Span) -> bool:
        ...
        if not all(template_data[key] is not None for key in Windows.template_keys):
            ...
            error_msg = f'Template Data Error, the following keys were missing from the VM quiesce data: ' \
                        f'{", ".join(missing_keys)}.'
        ...
        try:
            ...
            error = f'Exception occurred while attempting to quiesce VM #{vm_id} on {host_name}.'
