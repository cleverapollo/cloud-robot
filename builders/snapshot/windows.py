"""
builder class for windows snapshots

- gathers template data
- connects to the snapshot vm's server and builds the snapshot on the vm
...
class Windows(WindowsMixin):
    ...
    When we get to this point, we can be sure that the Snapshot
    is on a windows host
    """
    logger = logging.getLogger('robot.builders.snapshot.windows')
    template_keys = {
    ...
    def build(snapshot_data: Dict[str, Any], span: Span) -> bool:
        ...
        template_data = Windows._get_template_data(snapshot_data, child_span)
        child_span.finish()
        ...
        cmd = utils.JINJA_ENV.get_template('snapshot/hyperv/commands/build.j2').render(**template_data)
        child_span.finish()
        ...
        try:
            ...
            error = f'Exception occurred while attempting to build Snapshot #{snapshot_id} on {host_name}.'
            ...
            if response.std_out:
                ...
                Windows.logger.debug(f'Snapshot build command for Snapshot #{snapshot_id} generated stdout\n{msg}')
                built = 'successfully created snapshot' in msg
            ...
            if response.std_err and '#< CLIXML\r\n' not in response.std_err:
                ...
                error = f'Snapshot build command for Snapshot #{snapshot_id} generated stderr\n{msg}'
                snapshot_data['errors'].append(error)
                Windows.logger.error(error)
    ...
    def _get_template_data(snapshot_data: Dict[str, Any], span: Span) -> Optional[Dict[str, Any]]:
        """
        Given the snapshot data from the API, create a dictionary that contains all of the
        ...
        :param span: Span
        :returns: The data needed for the templates to build a Windows Snapshot
        """
        snapshot_id = snapshot_data['id']
        ...
        if host_name is None:
            error = f'Host name is not found for the server # {snapshot_data["server_id"]}'
            Windows.logger.error(error)
            snapshot_data['errors'].append(error)
            return None
