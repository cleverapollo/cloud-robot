...
SUCCESS_EMAIL_SUBJECT = 'Your VM "{name}" has been updated successfully!'
FAILURE_EMAIL_SUBJECT = 'Your VM "{name}" has failed to update'
...
def update_vm(vm_id: int):
    ...
    response = IAAS.vm.partial_update(token=Token.get_instance().token, pk=vm_id, data={'state': state.UPDATING})
