...
class Vr(VrMixin):
    """
    Class that handles the restarting of the specified VR
    """
    # Keep a logger for logging messages from this class
    logger = logging.getLogger('robot.restarters.vr')
    ...
    template_keys = {
        ...
        'management_ip',
        # The id of the Project that owns the VR being restarted
        'project_id',
    }

    @staticmethod
    def restart(vr_data: Dict[str, Any], span: Span) -> bool:
        """
        Commence the restart of a vr using the data read from the API
        ...
        if not all(template_data[key] is not None for key in Vr.template_keys):
            ...
            Vr.logger.error(
                f'Template Data Error, the following keys were missing from the VR restart data: '
                f'{", ".join(missing_keys)}',
            )
            span.set_tag('failed_reason', 'template_data_keys_missing')
            return False

        # If everything is okay, commence restarting the VR
        child_span = opentracing.tracer.start_span('generate_setconf', child_of=span)
        conf = utils.JINJA_ENV.get_template('vr/restart.j2').render(**template_data)
        child_span.finish()
