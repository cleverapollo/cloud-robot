...
class Vrf:
    ...
    logger = utils.get_logger_for_name('restarters.vrf')

    @staticmethod
    def restart(vrf: dict, password: str) -> bool:
        """
        Using the JunOS python library, this method attempts to restart a Virtual Router in a Physical Router using the
        config passed in from the dispatcher, and reports back to the dispatcher whether or not the restart succeeded
        :param vrf: The VRF data generated by the dispatcher
        :param password: The network password for connecting to the router
        :return: A flag stating whether the restart succeeded or not
        """
        Vrf.logger.info(f'Generating JunOS activate conf for VRF for Project #{vrf["idProject"]}')
        conf = utils.jinja_env.get_template('srx_restart_conf.j2').render(**vrf)
        # Log the restart conf to Debug
        Vrf.logger.debug(f'Generated restart conf for Project #{vrf["idProject"]}\n{conf}')
