...
class Windows(WindowsMixin):
    ...
    def restart(vm_data: Dict[str, Any]) -> bool:
        ...
        vm_restart_cmd = utils.JINJA_ENV.get_template('vm/windows/restart_cmd.j2').render(**template_data)

        # Open a client and run the two necessary commands on the host
        restarted = False
        try:
            response = Windows.deploy(vm_restart_cmd, host_name)
        except WinRMError:
            Windows.logger.error(
            ...
            if response.status_code == 0:
                ...
                restarted = f'{template_data["vm_identifier"]} Successfully Rebooted' in msg.decode()
            else:
